...
test("Battle Lab exposes a dedicated effect inspector next to the board", async () => {
  ...
  assert.match(html, /card-inspector-lab\.js\?v=0\.4\.77/);
  assert.match(html, /test-lab-tools\.css\?v=0\.4\.77/);
});

test("card inspector reads real Beyond Codex text and evolved text", async () => {
